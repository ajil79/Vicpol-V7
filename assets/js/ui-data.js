/* Traffic defects, evidence lists, render-all, persons/officers/signature pools, OCR availability helpers. */
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // Renders a plain-English "have you done X?" checklist above the preview so
  // recruits can see at a glance what's still missing. Shares the same required-
  // field logic as validateDraft() (reports.js) but shows done items too. CSS
  // (body.recruit-mode) controls whether the card is visible; this always keeps
  // its contents current so it's ready the moment recruit mode is turned on.
  function buildRecruitChecklist() {
    const type = state.reportType;
    const items = [];
    const add = (label, ok) => items.push({ label, ok: !!ok });

    // Primary subject — same rule as validateDraft's hasPrimarySubject()
    let subjectOk;
    if (type === "traffic_warrant") subjectOk = norm(state.offender.name || state.trafficWarrant.owner);
    else if (type === "vicpol_arrest" || type === "vicpol_warrant") subjectOk = norm(state.vicpolWarrant.warrantName || state.offender.name || state.vicpolWarrant.owner);
    else if (type === "field_contact") subjectOk = norm(state.fieldContact.name);
    else if (type === "search_seizure") subjectOk = norm(state.searchSeizure.name);
    else subjectOk = norm(state.offender.name);

    if (type === "vehicle_inspection") {
      const vi = state.vehicleInspection || {};
      add("Vehicle class selected", norm(vi.vehicleType));
      add("Checklist completed", norm(vi.vehicleType) && !validateDraft().some(w => /checklist/i.test(w)));
      add("Signature name", norm(state.sigName));
    } else if (type === "field_contact") {
      add("Person name", subjectOk);
      add("Officers listed", norm(state.officersList));
      add("Summary of events", norm(state.fieldContact && state.fieldContact.summary));
      add("Signature name", norm(state.sigName));
    } else if (type === "search_seizure") {
      add("Person name", subjectOk);
      add("Officers listed", norm(state.officersList));
      add("Search summary", norm(state.searchSeizure && state.searchSeizure.summary));
      add("Signature name", norm(state.sigName));
    } else {
      add("Subject / offender name", subjectOk);
      add("Charge or PIN selected", norm(state.chargesList) || norm(state.pinsList));
      if (type !== "traffic_warrant") add("Offender DOB", norm(state.offender.dob));
      if (type === "vicpol_arrest" || type === "vicpol_warrant") {
        const confirmed = (state.vicpolWarrant.idStatus || "").toUpperCase() === "CONFIRMED";
        add("ID basis (if ID confirmed)", !confirmed || norm(state.vicpolWarrant.idBasis));
      }
      add("Officers listed", norm(state.officersList));
      add("Preliminary time, date & location", norm(state.prelimTime) && norm(state.prelimDate) && norm(state.prelimLocation));
      add("Summary of events", norm(state.summary) || norm(state.vicpolWarrant && state.vicpolWarrant.details));
      add("Signature name", norm(state.sigName));
    }
    return items;
  }

  function updateRecruitChecklist() {
    const box = document.getElementById("recruitChecklist");
    if (!box) return;
    // Skip the work entirely when recruit mode is off (card is hidden anyway).
    if (!document.body.classList.contains("recruit-mode")) { box.innerHTML = ""; return; }
    let items;
    try { items = buildRecruitChecklist(); } catch (e) { box.innerHTML = ""; return; }
    const done = items.filter(i => i.ok).length;
    const total = items.length;
    const allDone = done === total;
    const esc = (typeof escapeHtml === "function") ? escapeHtml : (s => s);
    let html = '<h3>🎓 Recruit checklist ' +
      '<span class="' + (allDone ? "rc-done" : "") + '" style="margin-left:auto;font-weight:800">' + done + "/" + total + (allDone ? " ✓ ready" : "") + "</span></h3>";
    html += "<ul>" + items.map(i =>
      '<li class="' + (i.ok ? "rc-ok" : "rc-miss") + '"><span class="rc-icon">' + (i.ok ? "✓" : "•") + "</span><span>" + esc(i.label) + "</span></li>"
    ).join("") + "</ul>";
    box.innerHTML = html;
  }

  if (typeof window !== "undefined") window.updateRecruitChecklist = updateRecruitChecklist;
