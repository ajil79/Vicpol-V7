/* Event bindings, theme/now actions, OCR lab preview/apply flows, startup hooks. */
  ...
  function initUiBindings() {
    ...
    // Overlays plain-English hints (.recruit-hint) and shows the live checklist.
    // Persisted in localStorage so it stays on across reloads for a recruit.
    const recruitModeBtn = document.getElementById('recruitModeBtn');
    const applyRecruitMode = (on) => {
      document.body.classList.toggle('recruit-mode', on);
      if (recruitModeBtn) {
        recruitModeBtn.classList.toggle('recruit-on', on);
        recruitModeBtn.setAttribute('aria-pressed', on ? 'true' : 'false');
        recruitModeBtn.textContent = on ? '🎓 Recruit Mode ✓' : '🎓 Recruit Mode';
      }
      if (on) {
        // Open the writing-standards panel so the guidance is visible, not hidden.
        const gt = document.getElementById('guidelinesToggle');
        const gb = document.getElementById('guidelinesBody');
        if (gt && gb && !gb.classList.contains('open')) { gt.classList.add('open'); gb.classList.add('open'); gt.setAttribute('aria-expanded', 'true'); }
      }
      if (typeof updateRecruitChecklist === 'function') { try { updateRecruitChecklist(); } catch (e) {} }
    };
    if (recruitModeBtn && recruitModeBtn.dataset.bound !== '1') {
      recruitModeBtn.dataset.bound = '1';
      // Restore saved state
      let saved = false;
      try { saved = localStorage.getItem('vicpol_recruit_mode') === '1'; } catch (e) {}
      applyRecruitMode(saved);
      recruitModeBtn.addEventListener('click', () => {
        const on = !document.body.classList.contains('recruit-mode');
    ...
    // Inline "open the Day-to-Day Guide" links inside recruit hints etc.
    if (!document.body.dataset.toolPageJumpBound) {
      document.body.dataset.toolPageJumpBound = '1';
      document.addEventListener('click', (event) => {
        const jb = event.target.closest('[data-tool-page-jump]');
        if (!jb) return;
        event.preventDefault();
        showToolPage(jb.dataset.toolPageJump);
      });
    }
